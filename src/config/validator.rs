...
fn validate_display_config(config: &Config) -> Result<()> {
    if config.display.max_frameworks == 0 {
        return Err(ConfigError::ValidationError {
            message: "max_frameworks must be greater than 0".to_string(),
        }
        .into());
    }
    ...
    if config.display.framework_separator.is_empty() {
        return Err(ConfigError::ValidationError {
            message: "framework_separator cannot be empty".to_string(),
        }
        .into());
    }
...
fn validate_language(language: &ProjectIndicator) -> Result<()> {
    ...
    if language.files.is_empty() {
        return Err(anyhow::anyhow!("must have at least one file pattern"));
    }

    for file_pattern in &language.files {
        if file_pattern.trim().is_empty() {
            return Err(anyhow::anyhow!("file pattern cannot be empty"));
        }
    }

    // Validate color (basic hex color check)
    if !is_valid_hex_color(&language.color) {
        return Err(anyhow::anyhow!("invalid hex color: {}", language.color));
    }
    ...
    if language.priority == 0 {
        return Err(anyhow::anyhow!("priority must be greater than 0"));
    }
...
fn validate_framework(framework: &FrameworkDetector) -> Result<()> {
    ...
    if framework.name.trim().is_empty() {
        return Err(anyhow::anyhow!("framework name cannot be empty"));
    }

    // Validate priority
    if framework.priority == 0 {
        return Err(anyhow::anyhow!("framework priority must be greater than 0"));
    }
    ...
    if let Some(color) = &framework.color {
        if !is_valid_hex_color(color) {
            return Err(anyhow::anyhow!("invalid framework hex color: {}", color));
        }
    }
    ...
    match detection {
        ...
        DetectionType::ConfigFile { file, keys } => {
            if file.trim().is_empty() {
                return Err(anyhow::anyhow!("ConfigFile file path cannot be empty"));
            }
...
fn validate_non_empty_vec(vec: &[String], context: &str) -> Result<()> {
    ...
    for item in vec {
        if item.trim().is_empty() {
            return Err(anyhow::anyhow!("{} cannot contain empty strings", context));
        }
    }
...
fn validate_unique_language_names(languages: &[ProjectIndicator]) -> Result<()> {
    ...
    for language in languages {
        ...
        if !names.insert(lowercase_name.clone()) {
            return Err(ConfigError::ValidationError {
                message: format!("duplicate language name: {}", language.name),
            }
            .into());
        }
    }
