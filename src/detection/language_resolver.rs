...
use crate::types::{
    ConfidenceFactor, DetectionEvidence, EvidenceItem, MatchedFile, ProjectIndicator,
};
use std::sync::Arc;
...
impl LanguageResolver {
    ...
    pub fn detect_language_with_conflict_resolution_and_evidence(
        &self,
        ...
        for (idx, language) in languages.iter().enumerate() {
            let weighted_score =
                self.confidence_scorer
                    .calculate_language_score(language, matched_files, languages);
                ...
                for file in matched_files {
                    for pattern in &language.files {
                        if simple_wildcard_match(pattern, &file.filename) {
                            ...
                            evidence.add_language_evidence(item);
                        }
                    }
                }
...
mod tests {
    ...
    use crate::detection::matchers::test_helpers::helpers::{
        create_test_file, create_test_language_with_priority,
    };
    ...
    fn test_language_resolver_creation() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let result = resolver.detect_language_with_conflict_resolution_and_evidence(
            &empty_languages,
            &empty_files,
            &mut evidence,
        );
    ...
    fn test_detect_language_no_matches() -> Result<(), Box<dyn std::error::Error>> {
        let resolver = LanguageResolver::new();
        let languages = vec![
            Arc::new(create_test_language_with_priority(
                "Rust",
                vec!["Cargo.toml", "*.rs"],
                1,
            )),
    ...
    {
        let resolver = LanguageResolver::new();
        let languages = vec![Arc::new(create_test_language_with_priority(
            "Rust",
            vec!["*.rs"],
            1,
        ))];
    ...
    fn test_resolve_language_conflicts_clear_winner() -> Result<(), Box<dyn std::error::Error>> {
        let resolver = LanguageResolver::new();
        let languages = vec![
            Arc::new(create_test_language_with_priority("Rust", vec!["*.rs"], 1)),
            Arc::new(create_test_language_with_priority(
                "JavaScript",
                vec!["*.js"],
                2,
    ...
    fn test_confidence_tiers_high_confidence() -> Result<(), Box<dyn std::error::Error>> {
        let resolver = LanguageResolver::new();
        let languages = vec![
            Arc::new(create_test_language_with_priority("Medium", vec!["*.a"], 1)),
            Arc::new(create_test_language_with_priority("High", vec!["*.b"], 2)),
        ];
        let candidates = vec![(0, 0.7, 1.0), (1, 0.9, 1.0)];
    ...
    fn test_evidence_tracking() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert!(
            !evidence.language_evidence.is_empty(),
