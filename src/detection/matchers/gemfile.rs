    ...
    pub fn detect_frameworks(
        ...
        for framework in frameworks {
            if let DetectionType::GemSpec { gems } = &framework.detection {
                ...
                if !found_gems.is_empty() {
                    let confidence = Self::calculate_confidence(gems, &found_gems);
        ...
        matches.sort_by(|a, b| {
            b.confidence
                ...
                .then_with(|| a.framework.priority.cmp(&b.framework.priority))
        });

        Ok(matches)
    }

    /// Calculate confidence based on gem matches
    fn calculate_confidence(required_gems: &[String], found_gems: &[String]) -> f32 {
        if required_gems.is_empty() {
            return 0.0;
        }
        found_gems.len() as f32 / required_gems.len() as f32
    }
}
...
"#;
    ...
    fn test_confidence_calculation() {
        assert_eq!(
            GemfileMatcher::calculate_confidence(&["rails".to_string()], &["rails".to_string()]),
            1.0
        );
        assert_eq!(
            GemfileMatcher::calculate_confidence(
                &["rails".to_string(), "sidekiq".to_string()],
                &["rails".to_string()]
            ),
            0.5
        );
        assert_eq!(GemfileMatcher::calculate_confidence(&[], &[]), 0.0);
    }
}
