...
impl ComposerJsonMatcher {
        ...
        for framework in frameworks {
            if let DetectionType::ComposerJson { packages } = &framework.detection {
                ...
                if !found_packages.is_empty() {
                    let confidence = Self::calculate_confidence(packages, &found_packages);
    ...
    /// Calculate confidence based on package matches
    fn calculate_confidence(required_packages: &[String], found_packages: &[String]) -> f32 {
        if required_packages.is_empty() {
            return 0.0;
        }
        found_packages.len() as f32 / required_packages.len() as f32
    }
}
...
mod tests {
    ...
    fn test_confidence_calculation() {
        assert_eq!(
            ComposerJsonMatcher::calculate_confidence(
        ...
        assert_eq!(ComposerJsonMatcher::calculate_confidence(&[], &[]), 0.0);
    }
